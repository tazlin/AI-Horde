...
class ImageAsyncStatus(Resource):
    ...
    def get(self, id=""):
        ...
        t0 = time.monotonic()
        outcome = "ok"
        with pyroscope_tag(endpoint="status", gentype="image"), logfire.span(
            "horde.generate.status", gentype="image", wp_id=id
        ):
            try:
...
class ImageAsyncCheck(Resource):
    ...
    def get(self, id):
        ...
        t0 = time.monotonic()
        outcome = "found"
        with pyroscope_tag(endpoint="check", gentype="image"), logfire.span(
            "horde.generate.check", gentype="image", wp_id=id
        ):
            try:
                self.args = self.get_parser.parse_args()
                ...
                check_outcomes.add(
                    1, {"horde.gentype": "image", "horde.outcome": outcome}
                )
